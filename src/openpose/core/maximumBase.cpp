...
namespace op
{
            ...
            for (auto n = 0; n < num; n++)
            {
                for (auto c = 0; c < channels; c++)
                {
                    ...
                    for (auto part = 0; part < numberParts; part++)
                    {
                        ...
                        // Option a - 6.3 fps
                        const auto sourceIndexIterator = std::max_element(sourcePtrOffsetted, sourcePtrOffsetted + imageOffset);
    ...
    template void maximumCpu(float* targetPtr, const float* const sourcePtr, const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize);
    template void maximumCpu(double* targetPtr, const double* const sourcePtr, const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize);
}
